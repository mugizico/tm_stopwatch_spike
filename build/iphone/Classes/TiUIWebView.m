...
static NSString * const ktm_stopwatch_spikeJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_tm_stopwatch_spikeInjection
{
	...
	[html appendString:ktm_stopwatch_spikeJavascript];
...
{
	// attempt to make well-formed HTML and inject in our tm_stopwatch_spike bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _tm_stopwatch_spikeInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to tm_stopwatch_spike support.",url);
				}
			}
