...
@implementation tm_stopwatch_spikeObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_tm_stopwatch_spike);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any tm_stopwatch_spike patches in JS before a context is loaded... nice for 
...
{
	// create tm_stopwatch_spike global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_tm_stopwatch_spike = [[tm_stopwatch_spikeObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_tm_stopwatch_spike];
	
	NSString *_tm_stopwatch_spikeNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _tm_stopwatch_spikeNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_tm_stopwatch_spike valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.tm_stopwatch_spike.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
